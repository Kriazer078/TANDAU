...
class GrantChanceService {
  ...
  static const Map<MajorCategory, Set<EntSubjectPair>> allowedSubjectPairs = {
    MajorCategory.it: {
    ...
    MajorCategory.law: {EntSubjectPair.historyLaw},
    MajorCategory.medicine: {EntSubjectPair.bioChemistry},
    MajorCategory.agriculture: {
      EntSubjectPair.bioChemistry,
      EntSubjectPair.bioGeography,
      EntSubjectPair.mathPhysics,
    },
    MajorCategory.arts: {EntSubjectPair.creativeExams},
  ...
  String? validateSubjectPair(MajorCategory category, EntSubjectPair pair) {
    ...
    if (!allowed.contains(pair)) {
      final String allowedNames = allowed.map((e) => e.displayName).join(', ');
  ...
  GrantChanceResult calculate({
    ...
    if (!hasGrants) {
      return GrantChanceResult(
        chancePercent: 0,
        riskLevel: RiskLevel.unknown,
        verdict: 'Данный вуз не участвует в конкурсе на государственные гранты',
        details: [
    ...
    if (subjectPair != null) {
      final String? pairError = validateSubjectPair(majorCategory, subjectPair);
      if (pairError != null) {
        return GrantChanceResult(
          chancePercent: 0,
      ...
      for (int i = 0; i < perSubjectScores.length; i++) {
        if (perSubjectScores[i] < minPerSubjectScore) {
          return GrantChanceResult(
            ...
            riskLevel: RiskLevel.critical,
            verdict:
                'Один из предметов ЕНТ ниже минимума ($minPerSubjectScore)',
            details: [
    ...
    if (categoriesRequiringSpecialExam.contains(majorCategory) &&
      ...
      return GrantChanceResult(
        ...
        recommendations: ['Зарегистрируйтесь и сдайте $examName экзамен'],
        entThreshold: _getThreshold(universityId, majorCategory),
        dataYear: '2026',
      );
    }
    ...
    if (entScore < threshold) {
      ...
      final int maxPossibleSurplus = (maxEntScore - targetScore).clamp(1, 140);
      baseChance = 40 + (surplus / maxPossibleSurplus * 45).clamp(0.0, 45.0);
  ...
  GrantChanceResult calculateBySpecialty({
    ...
    if (subjectPair != null) {
      ...
      if (pairError != null) {
        return GrantChanceResult(
          ...
          details: ['❌ $pairError'],
          recommendations: ['Выберите допустимую комбинацию предметов'],
          entThreshold: minPassingScore,
          dataYear: '2025/2026',
        );
      }
    ...
    if (entScore < threshold) {
      ...
      return GrantChanceResult(
        ...
        details: details,
        recommendations: ['Поднимите балл ЕНТ минимум до $threshold'],
        entThreshold: threshold,
        dataYear: '2025/2026',
      );
    }
    ...
    // 🔥 Предупреждение о военной кафедре
    if (!hasMilitaryDepartment) {
      recommendations.add('⚠️ В данном вузе отсутствует военная кафедра');
    }
