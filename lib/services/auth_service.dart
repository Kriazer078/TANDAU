  ...
  Future<String?> redeemPromoCode(String code) async {
    try {
      ...
      final result = await _firestore.runTransaction<String?>((
        ...
        final int maxUses = data['maxUses'] ?? 1;
        final int currentUses = data['currentUses'] ?? 0;
        final List<dynamic> usedBy = data['usedBy'] ?? [];
        final String plan = data['plan'] ?? 'pro'; // 'pro' or 'premium'
        ...
        // Determine tokens
        int newTokens = plan == 'premium' ? 1000 : 100;
        final now = DateTime.now().toUtc();
        ...
        transaction.update(userRef, {
          'subscriptionPlan': plan,
          'aiTokensRemaining': newTokens,
          'lastTokenResetDate': now,
        });

        return 'SUCCESS:$plan:$newTokens';
      });

      if (result != null && result.startsWith('SUCCESS:')) {
        final parts = result.split(':');
        final plan = parts[1];
        final newTokens = int.parse(parts[2]);

        currentUser.value = currentUser.value!.copyWith(
          subscriptionPlan: plan,
          aiTokensRemaining: newTokens,
          lastTokenResetDate: DateTime.now().toUtc(),
        );
      ...
      debugPrint('Redeem promo code error: $e');
      return 'Ошибка при активации промокода. Возможно, нет доступа базы данных.';
    }
  }
