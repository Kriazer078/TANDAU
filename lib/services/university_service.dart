...
import '../data/universities.dart' show universitiesList;
...
class UniversityService {
  ...
  Future<List<University>> getAllUniversities({
    ...
    try {
      ...
      // 🔄 Fallback: if Firestore is empty, use local data
      if (universities.isEmpty) {
        debugPrint('⚠️ Firestore empty — using local universitiesList (${universitiesList.length})');
        _cachedAllUniversities = universitiesList;
      ...
      // 3c. Local hardcoded fallback
      debugPrint('📦 Using local universitiesList fallback (${universitiesList.length})');
      return universitiesList;
    }
  }
  ...
  /// ⚠️ Удаление ВСЕХ университетов (Только для Админа)
  Future<bool> deleteAllUniversities() async {
    final result = await _firestoreService.deleteAllUniversities();
    if (result) clearCache();
    return result;
  }

  /// Загрузка университета вместе с его специальностями в рамках одной транзакции
  Future<bool> uploadUniversityWithSpecialties(University university, List<Specialty> specialties) async {
    final result = await _firestoreService.uploadUniversityWithSpecialties(university, specialties);
    if (result) clearCache();
    return result;
  }
