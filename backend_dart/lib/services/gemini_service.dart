...
### LOGIC: АЛГОРИТМ 4-Х ВУЗОВ
На основе балла пользователя ($untScore) ты ОБЯЗАН распределить рекомендации так:
1. DREAM (Риск): Вуз/Специальность, где балл прошлого года на 3-7 выше текущего балла пользователя.
2. TARGET 1 (Высокий шанс): Балл примерно совпадает с результатами прошлого года.
3. TARGET 2 (Надежный): Балл на 5-10 ниже текущего балла пользователя.
4. SAFETY (Гарантия): Вуз с самым низким порогом или программа «Серпін».

### RESPONSE STRUCTURE
Отвечай строго по шаблону (используй Markdown-форматирование):
1. **Краткий анализ ситуации** (хватает ли баллов на выбранную группу ОП). Будь предельным честным.
2. **Алгоритм 4-х вузов (Твоя стратегия поступления)**. Распиши 4 пункта с конкретными вузами РК. 
3. **Секретные лайфхаки / Квоты**. Обязательно дай 2-3 неочевидных совета по поступлению в 2026 году.
4. **Алгоритм заполнения бланка в НЦТ**. Укажи, в каком порядке лучше ставить университеты.

### CONSTRAINTS (Ограничения)
- Если балл пользователя ниже порогового (<50), мягко предложи пересдачу или варианты платного обучения/колледжа.
- НЕ выдумывай баллы. Если данных нет, скажи: "Рекомендую уточнить актуальный порог в карточке вуза в приложении TANDAU".
- Тон: Экспертный, вдохновляющий, но честный (без ложных надежд).
- Не используй воду. Давай четкую аналитику, применимую именно в Казахстане в 2026 учебном году.
''';
    return _generate(prompt);
  }
}
